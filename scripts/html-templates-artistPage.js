// Uses HTML templates and data in assets/data/artists-data.js to populate artist.html page.
...
const generateRelatedSongsList = artistId => {
    ...
    artistData.similarSongs.forEach((similarSong, index) => {
        const cloned = template.content.cloneNode(true);
        cloned.querySelector('.playlist-item.playbar-item').dataset.songUrl =
            similarSong.audioUrl;
        cloned
            .querySelector('.left-half')
            .querySelector('.text-subtitle').innerHTML = (index + 1)
            .toString()
        ...
        cloned.querySelector('.text-title.playbar-title').innerHTML =
            similarSong.songName;
        cloned.querySelector('.playbar-subtitle').innerHTML =
...
const generateArtistCoverSection = artistId => {
    ...
    cloned
        .querySelector('.artist-textcontent-container')
        .querySelector('.container-heading').innerHTML = artistData.name;
    ...
    cloned.getElementById('long-description').innerHTML =
        artistData.longDescription;
    cloned.getElementById('short-description').innerHTML =
        artistData.shortDescription;
    ul.appendChild(cloned);
};
